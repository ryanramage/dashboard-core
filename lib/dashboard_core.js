...
exports.install_app = function(remote_app_details, new_db_name, update_status_function, callback) {
    ...
    async.waterfall([
        ...
        function(callback) {
            update_status_function('Configuring App', '80%');
            var couch_db = $.couch.db(new_db_name);
            copyDoc(couch_db, remote_app_details.doc_id, '_design/' + remote_app_details.doc_id, callback);
        },
        function(callback) {
            update_status_function('Cleaning Up', '90%');
            var couch_db = $.couch.db(new_db_name);
            deleteDoc(couch_db, new_db_name, remote_app_details.doc_id, callback);
        },
        function(callback) {
            update_status_function('Recording Install', '95%');
        ...
        function(callback) {
            update_status_function('Setting security', '98%', true);
            var couch_db = $.couch.db(new_db_name);
            exports.addDBReaderRole(new_db_name, '_admin', callback);
        }
...
function addDashboardUrl(markets) {
    var dashboardUrl = thisDashboardUrl(window.location);
    markets = _.map(markets, function(row) {
...
function copyDoc(couch_db, from_doc_id, to_doc_id, callback) {
    couch_db.copyDoc(
       from_doc_id,
       {
            error: function() {
